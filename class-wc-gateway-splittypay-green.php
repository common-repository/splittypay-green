...
/**
 * Plugin Name: Green pay
 * Description: A payment gateway for Splittypay Green Checkout.
 * Version: 1.1.8
 * Author: Splitty Pay
 * Author URI: https://www.splittypay.com/
 * Developer: Federico Gatti
 * Text Domain: woocommerce-gateway-splittypay-green-checkout
 ...
 */
function wc_splittypay_green_add_to_gateways( $gateways ) {
    $gateways[] = 'WC_Splittypay_Green_Gateway';
    return $gateways;
}
add_filter( 'woocommerce_payment_gateways', 'wc_splittypay_green_add_to_gateways' );
 ...
 */
function wc_splittypay_green_gateway_plugin_links( $links ) {
    $plugin_links = array(
        '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=checkout&section=splittypay_green_gateway' ) . '">' . __( 'Configure', 'woocommerce' ) . '</a>'
    );
    return array_merge( $plugin_links, $links );
}
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'wc_splittypay_green_gateway_plugin_links' );
 ...
 */
add_action( 'plugins_loaded', 'wc_splittypay_green_gateway_init', 11 );
function wc_splittypay_green_gateway_init() {
    class WC_Splittypay_Green_Gateway extends WC_Payment_Gateway {

        private $config = array(
            "baseUrl" => 'https://app.green.splittypay.com/#',
            "postOrderUrl" => 'https://api-prod.green.splittypay.com/orders?token=',
            "port"    => '',
        );
        ...
        public function __construct() {
            // Config
            $this->id                 = 'splittypay_green_gateway';
            /*$this->icon               = apply_filters('woocommerce_splittypay_green_icon', WC_HTTPS::force_https_url( WC()->plugin_url() . '-splittypaygreenpayments/assets/images/splittypay.png' ));*/
            $this->has_fields         = false;
            $this->method_title       = __( 'Green Pay', 'woocommerce' );
            ...
            if ( $this->testmode ) {
                $this->config['baseUrl'] = 'https://sandbox.dev.green.splittypay.com/#';
                $this->config['postOrderUrl'] = 'https://api.dev.green.splittypay.com/orders?token=';
                $this->config['port'] = '';
            ...
            add_action( 'woocommerce_api_wc_splittypay_green_gateway', array($this, 'handle_callback') ); 
        }
        ...
        public function init_form_fields() {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default'     => __( 'Paga in modo sostenibile con Green Pay', 'woocommerce' ),
                    'desc_tip'    => true,
                ),
                ...
                'description' => array(
                    ...
                    'default'     => __( 'Il metodo di pagamento sostenibile di Splittypay. Grazie a questo metodo potrai adottare un alveare con l\'1% del tuo ordine.', 'woocommerce' ),
                    'desc_tip'    => true,
                ),

                'testmode'    => array(
                    'title'       => __( 'Splittypay - Green sandbox', 'woocommerce' ),
                    'type'        => 'checkbox',
                    'label'       => __( 'Abilita la sandbox di Splittypay e ricevi pagamenti solo in modalità TEST', 'woocommerce' ),
                    'default'     => 'no',
                    'description' => sprintf( __( 'La sandbox è un ambiente di TEST di Splittypay - Green Pay', 'woocommerce' ), 'https://sandbox.splittypay.it/'),
                ),
        ...
        public function process_payment( $order_id ) {
            ...
            $order_data = array(
                'content' => array (
                    ...
                    'order_ref' => WC()->api_request_url( 'wc_splittypay_green_gateway' ),
...
/*
add_action( 'woocommerce_before_add_to_cart_form', 'wc_splittypay_green_gateway_add_product_desc' );

function wc_splittypay_green_gateway_add_product_desc(){
    global $product;
	echo 'O paga '.number_format($product->get_price()/3, 2).'€ al mese con Splittypay';
}*/
